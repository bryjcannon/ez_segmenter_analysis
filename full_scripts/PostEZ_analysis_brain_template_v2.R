...
# create FlowSet from fcs files
  fcsNames = c()
  for (run in ezRun) {
    fcsNames = c(fcsNames, list.files(path = paste0(run,'/','fcs_all'), full.names = T, pattern = ".fcs"))
  }
  init_flowSet = read.flowSet(files = fcsNames, alter.names = T, transformation = FALSE, emptyValue = FALSE, truncate_max_range = FALSE)
...
# add columns to each file denoting point number, type of tissue, name of objects
  ...
  for (name in point_names) {
    region_names = c(region_names, init_flowSet@frames[[name]]@description[["FILENAME"]])
  }
