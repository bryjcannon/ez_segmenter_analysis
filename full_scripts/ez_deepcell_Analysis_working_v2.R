...
##### 1a) LOAD ezSeg DATA #####
  ...
  ez_folder <- '/Volumes/BryJC_Stanford/For_Ez_Segmentor/HiResTMA/AD_Panel/Feb2019/'
  setwd(ez_folder)
  # specify run folders
  ezRun <- c('ezSegResults_tma_ADpart2_pt12_18+20_22', 'ezSegResults_tma_ADpart2_pt19')
  ezRunsShort = c('AD', 'AD') # if same region for multiuple runs, write that region # run times
  object_types <- c('amyloidopathy', 'microglia_process', 'tauopathy', 'vessel_CD31', 'vessel_MCT1')
...
# create data.frame containing single object data from csv files - for each run and each object type
  ...
  for (run_index in 1:length(ezRun)){
    ...
    for (obj_type in object_types) {
      ...
      # if you need to grab centroid data from your files (ez_2.0 will have this natively)
      # csv_centroid_names <- list.files(path = paste0(ezRun[run_index],'/','objects_points'), recursive = T, full.names = T, pattern = paste0(obj_type, "_centroid.csv"))
      # csv_names <- mixedsort(csv_centroid_names)
      # obj_data_raw_centroids <- lapply(csv_centroid_names, read.csv) %>% bind_rows()
      # centroid_x <- cbind(obj_data_raw, obj_data_raw_centroids$centroid_x)
      # centroid_y <- cbind(obj_data_raw, obj_data_raw_centroids$centroid_y)
      
      #####
      # centroids, majoraxis length, pixelList
      mat_names <- list.files(path = paste0(ezRun[run_index],'/','objects_points'), recursive = T, full.names = T, pattern = paste0(obj_type, "_objData.mat")) # read in mat files for object type
      mat_names <- mixedsort(mat_names)
      obj_properties <- lapply(mat_names, readMat)
      # pull out mapped object ids as a matrix (defaults to image dimensions)
      obj_centroids <- obj_properties$mapped.obj.ids
      obj_majorAxisLength <- obj_properties$mapped.obj.ids
      obj_PixelList <- obj_properties$mapped.obj.ids
      #####
...
##### FELIX CODE SNIPPET #####
  ...
  ff_1 <- flowFrame(exprs = data.matrix(data_all[,channels_cluster]), desc = list(FIL = 1))
  ff_1
  # run FlowSOM (with set.seed for reproducibility)
  set.seed(123)
  out_fSOM <- FlowSOM::ReadInput(flowFrame_cluster, transform = FALSE, scale = FALSE, compensate = FALSE)
  out_fSOM <- FlowSOM::BuildSOM(out_fSOM, colsToUse = clustering_markers, xdim=20, ydim=20)
...
flowFrame_cluster <-  flowFrame(exprs = as.matrix(subset(master_data, obj_type_id == 'cell', select = panel)), desc = list(FIL = 1))
